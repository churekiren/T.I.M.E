...
export function BureauConsoleLayout() {
  const { sessionId } = useWorkingSession()
  const wallUrl = `${import.meta.env.BASE_URL}wall?session=${encodeURIComponent(sessionId)}`
  return <div className="bureau-console"><BureauSidebar wallUrl={wallUrl} /><div className="bureau-console-main"><StaffSessionBar label="BUREAU CONSOLE" /><div className="bureau-console-content"><Outlet /></div></div></div>
}

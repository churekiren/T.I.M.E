...
export const wallRepository = {
  ...
  subscribe(sessionId, callback) {
    try {
      ...
      const channel = client.channel(`wall:${sessionId}`).on('postgres_changes', { event: '*', schema: 'public', table: 'wall_entries', filter: `session_id=eq.${sessionId}` }, (payload) => callback(payload)).subscribe()

...
export function Access() {
  ...
  if (access.state === 'WAITING' || access.state === 'UNASSIGNED') return <section className="access-gate access-waiting"><img src={assetUrl('assets/time-emblem.png')} alt="T.I.M.E. 正式局徽" /><div className="waiting-signal"><Radio /></div><span className="eyebrow">TEMPORAL LINK ESTABLISHED</span><h1>AWAITING AUTHORIZATION</h1><h2>時界連線已建立</h2><div className="access-meta"><span>CREDENTIAL<strong>{access.shortCode}</strong></span><span>LINK STATUS<strong>STANDBY</strong></span><span>CHANNEL<strong>SECURE</strong></span></div><p>等待時界局開放身份建檔權限。</p><small className="waiting-reconnect">此憑證已進入待命序列。</small>{error && <small className="waiting-reconnect">連線正在自動恢復；憑證待命狀態不受影響。</small>}</section>

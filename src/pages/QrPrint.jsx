...
export function QrPrint() {
  ...
  return <main className="qr-print-page">
    ...
    {!localOnlyOrigin && <section className="qr-print-sheet" aria-label="A4 QR 貼紙排版">{tokens.map((access) => <article className="qr-sticker" key={access.id}><QrCode value={access.accessUrl} label={access.shortCode} /><strong>{access.shortCode}</strong></article>)}</section>}
  </main>
}
